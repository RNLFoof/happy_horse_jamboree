...
local particles = assert(SMODS.load_file("behavior\\vanilla\\particles.lua"))()
...
_obj_0[#_obj_0 + 1] = testing.TestBundle("particles", {
	testing.Test("init_after taking from both the provided and default config", {
		function()
			...
			return testing.assert_eq(dummy.hhj_config.gravity_direction, 270)
		end
	}),
	testing.Test("init_after on subconfig", {
		function()
			...
			testing.assert_eq(dummy.hhj_config.starting_range.min_x, 69)
			return testing.assert_eq(dummy.hhj_config.starting_range.max_x, G.CARD_W / 2)
		end
	})
})
