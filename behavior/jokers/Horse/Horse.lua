...
local round = useful_things.round
local required_horse_name_count = 40
local horse_names = {
	"Chestnut",
	"Peanut",
	"Acorn",
...
local neigh
neigh = function()
	return play_sound('hhj_neigh', 0.9 + math.random() * 0.2, 1)
end
local index = 0
		...
		for _, bonus_3 in ipairs((function()
			...
			for _, jack in ipairs({
				...
				if bonuses_for_this_horse.chips > 0 then
					horse_ability_description[#horse_ability_description + 1] = "{" .. tostring(scale) .. ",C:chips}+" .. tostring(bonuses_for_this_horse.chips) .. "{" .. tostring(scale) .. "} chips"
				end
				if bonuses_for_this_horse.mult > 0 then
					horse_ability_description[#horse_ability_description + 1] = "{" .. tostring(scale) .. ",C:mult}+" .. tostring(bonuses_for_this_horse.mult) .. "{" .. tostring(scale) .. "} Mult"
				end
				if bonuses_for_this_horse.money > 0 then
					horse_ability_description[#horse_ability_description + 1] = "{" .. tostring(scale) .. ",C:attention}" .. tostring(bonuses_for_this_horse.money) .. "{" .. tostring(scale) .. ",C:green} #1#/2{" .. tostring(scale) .. "} chance" .. tostring(bonuses_for_this_horse.money > 1 and 's' or '') .. " for {" .. tostring(scale) .. ",C:money}$1{}"
				end
				if bonuses_for_this_horse.luck > 0 then
					horse_ability_description[#horse_ability_description + 1] = "{" .. tostring(scale) .. ",C:green}HORSE LUCK X" .. tostring(bonuses_for_this_horse.luck) .. "{" .. tostring(scale) .. "}"
				end
				scale = "s:0.5"
				local _list_0 = {
					...
					"{" .. tostring(scale) .. ",C:inactive}and have varied abilities"
				}
				...
				local horse_name
				if index <= #horse_names then
					horse_name = horse_names[index]
				else
					horse_name = "Unnamed Horse (" .. tostring(index) .. ")"
				end
				...
				local horse_joker = SMODS.Joker({
					...
					loc_vars = function(self, info_queue, card)
						if bonuses_for_this_horse.luck > 0 then
							...
							info_queue[#info_queue + 1] = G.P_TAGS.tag_hhj_horse_luck
						end
						return {
							vars = {
								G.GAME and G.GAME.probabilities.normal or 1
					...
					calculate = function(self, card, context)
						local horse = card.ability
						if (((horse.jack == false) and context.joker_main and context.cardarea == G.jokers) or ((horse.jack == true) and context.individual and context.cardarea == G.play and context.other_card:get_id() == 11)) then
							local money_earned = 0
							for _ = 1, horse.bonuses.money do
								if pseudorandom('horse_money') < G.GAME.probabilities.normal / 2 then
									money_earned = money_earned + 1
								end
							end
							for _ = 1, horse.bonuses.luck do
								if pseudorandom('horse_luck') < G.GAME.probabilities.normal / 20 then
...
local _anon_func_0 = function(self)
	local _obj_0 = self.config.center
	if _obj_0 ~= nil then
		local _obj_1 = _obj_0.config
		if _obj_1 ~= nil then
			return _obj_1.is_horse
		end
		return nil
	end
	return nil
end
useful_things.wrap_method(Card, "hover", function(...)
	local self = ...
	if _anon_func_0(self) then
		return neigh()
	end
end)
local ref = Card.set_edition
...
local wrapper
wrapper = function(self, edition, immediate, silent)
	ref(self, edition, immediate, silent)
	if _anon_func_1(self) ~= nil then
		local horse_joker = self.config.center
		local the_sprite_is_in_here = self.children.center
		local change_pos_to
		...
		if horse_joker.pos ~= change_pos_to then
			...
			return the_sprite_is_in_here:set_sprite_pos(horse_joker.pos)
		end
	end
end
do
