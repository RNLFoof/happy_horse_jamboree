...
local adjust_add
adjust_add = function(rot_amount)
	return rot_amount * 2
end
return useful_things.wrap_method(Card, "juice_up", nil, function(self, scale, rot_amount)
	if G.SETTINGS.paused then
		return
	end
	local _list_0 = SMODS.find_card("j_hhj_shakedown", false)
	for _index_0 = 1, #_list_0 do
		local joker = _list_0[_index_0]
		rot_amount = math.abs(rot_amount and rot_amount or 0.16)
		scale = scale and scale * 0.4 or 0.11
		local add = adjust_add(rot_amount)
		local add_unrounded = add
		add = useful_things.round(add, 3)
		local adding_cutoff = adjust_add(math.max(0.03))
		if add <= adding_cutoff or add_unrounded <= adding_cutoff then
			return
		end
		local args = { }
		local draw_layer = -1
		local attach_to = UIBox({
			T = {
				0,
				0,
				0,
		...
		self.childParts2 = Particles(G.CARD_W / 2, G.CARD_H / 2, 0, 0, {
			...
			hhj_config = {
				...
				fixed_scale = {
					min = 0.2 * (1 - scale_variance),
					max = 0.2 * (1 + scale_variance)
				},
				min_speed = 2
			}
		})
		...
		local _obj_0 = joker.ability
		_obj_0.chips = _obj_0.chips + add
		attention_text({
			text = "+" .. tostring(add),
